...
if(isset($_GET["id"])){
    $id = $_GET["id"];
    $sqli = "select * from `returned` where id = $id";
    ...
    <marquee><h1>Welcome to the Library</h1></marquee>
        <div class="row justify-content-center">
            <div class="col-md-8 form-container  py-3">
                <h1 class="text-center ">Books Not Returned</h1>
                <form action="update_not_returned_action.php" method="POST">
                <input type="hidden" value="<?php echo $data["id"];    ?>" name="id" class="form-control" id="id"   required>
                    <div class="my-2">
                        ...
                        <select id="student_id" name="student_id" class="form-control" required>
                            ...
                            ?>
                            <option <?php echo $row3['student_id'] == $data['student_id'] ? "selected" : "" ?> value="<?php echo $row3["student_id"]; ?>"><?php echo $row3["student_id"];  ?></option>
                    ...
                    <div class="form-group ">
                        ...
                        <select id="book_id" name="book_id" class="form-control" required>
                            ...
                            ?>
                            <option <?php echo $row['id'] == $data['book_id'] ? "selected" : "" ?> value="<?php echo $row["id"]; ?>"><?php echo $row["name"];  ?></option>
                    ...
                    <div class="form-group ">
                        <label for="inputState" class="mb-2">Status :</label>
                        <select id="status" name="status" class="form-control" required>
                            <!-- <option selected>   --Selected--</option> -->
                            <option <?php echo $data['status'] == "Returned" ? "selected" : "" ?>>Returned</option>
                            <option <?php echo $data['status'] == "Not Returned" ? "selected" : "" ?>>Not Returned</option>
                            
                        </select>
                    </div>
                    <div class="my-2">
                        <label for="studentName" class="form-label">Expiry Date :</label>
                        <input type="date" value="<?php echo $data["expiry_date"];    ?>" name="expiry_date" class="form-control" id="expiry_date" placeholder="Enter Expiry Date"  required>
                    </div>
